...
async function main() {
  ...
  if (!version) {
    // Increment patch version if not explicitly passed
  ...
  if (!fs.existsSync(indexPath)) {
    throw new Error(`❌ www/index.html not found! Build may have failed.`);
  }

  // 2. Package www/ into zip
  ...
  const releaseId = `partner-rel-${version}-${Date.now().toString(36)}`;
  ...
  // Also update package.json version in oneapp_partner
  pkg.version = version;
